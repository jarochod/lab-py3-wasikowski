# R190. Requests - aplikacja przeliczająca waluty
...
class CurrencyConverter:
    ...
    def create_widgets(self):
        ...
        self.usd_label = ttk.Label(self.root, text="USD: N/A")
        self.usd_label.grid(column=0, row=1, padx=10, pady=10, sticky="W")
        self.eur_label = ttk.Label(self.root, text="EUR: N/A")
        self.eur_label.grid(column=0, row=2, padx=10, pady=10, sticky="W")
        self.jpy_label = ttk.Label(self.root, text="JPY: N/A")
    ...
    def calculate(self, event=None):
        ...
        try:
            # Pobranie kwoty w PLN od użytkownika (zamiana przecinka na kropkę)
            amount_pln = float(self.pln_entry.get().replace(',', '.'))
        except ValueError:
            # Jeśli pole jest puste lub błędne, ustaw etykiety na N/A
            for label, currency in [(self.usd_label, 'USD'),
                                    (self.eur_label, 'EUR'),
                                    (self.jpy_label, 'JPY')]:
                label.config(text=f"{currency} ({self.currency_rates[currency]} PLN): N/A")
            return
