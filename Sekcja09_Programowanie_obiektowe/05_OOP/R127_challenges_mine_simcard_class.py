...
class SimCard:
    ...
    def addContact(self, name: str, telephone: int) -> None:
        if not isinstance(name, str) or not isinstance(telephone, int):
            return  # Można też rzucić wyjątek
    ...
    def showContacts(self) -> None:
        for c in self.contacts:
            print(f"{c['name']} {c['telephone']}")

# Test
sim = SimCard()
sim.addContact("Ola", 987654321)
sim.addContact("Adam", 12345678)
# sim.addContact(100, 92345678)  # Zostanie zignorowane, bo name nie jest str
# sim.addContact("Kasia", "numer")  # Zostanie zignorowane, bo telephone nie jest int
sim.showContacts()
